...
def slice(a, start=None, stop=np._NoValue, step=None, /):
    ...
    >>> b = np.array(['hello world', '帠庰庣帢 ?怷? 庥??弮庰', '雿末銝?', '?? ??'],
    ...              dtype=np.dtypes.StringDType())
    >>> np.strings.slice(b, -2)
    array(['hello wor', '帠庰庣帢 ?怷? 庥??', '雿末', '??'], dtype=StringDType())

    >>> np.strings.slice(b, -2, None)
    array(['ld', '弮庰', '銝?', ' ??'], dtype=StringDType())

    >>> np.strings.slice(b, [3, -10, 2, -3], [-1, -2, -1, 3])
    array(['lo worl', ' ?怷? 庥??', '銝?, '?? ??'], dtype=StringDType())

    >>> np.strings.slice(b, None, None, -1)
    array(['dlrow olleh', '庰弮??庥 ?怷? 帢庣庰帠', '??憟賭?', '?? ??'],
          dtype=StringDType())
