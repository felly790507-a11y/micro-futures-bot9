...
def test_isnumeric_unicode(method, expected, any_string_dtype):
    # 0x00bc: 翹 VULGAR FRACTION ONE QUARTER
    # 0x2605: ??not number
    # 0x1378: ??ETHIOPIC NUMBER SEVENTY
    # 0xFF13: 嚗?Em 3  # noqa: RUF003
    ser = Series(
        ["A", "3", "糧", "翹", "??, "??, "嚗?, "four"],  # noqa: RUF001
    ...
    if (
        ...
        # pyarrow 21+ considers 翹 and ??as a digit, while python does not
...
def test_isnumeric_unicode_missing(method, expected, any_string_dtype):
    values = ["A", np.nan, "翹", "??, np.nan, "嚗?, "four"]  # noqa: RUF001
...
@pytest.mark.parametrize(
    "form, expected",
    [
        ("NFKC", ["ABC", "ABC", "123", np.nan, "?Ｕ??]),
        ("NFC", ["ABC", "嚗∴慰嚗?, "嚗?嚗?, np.nan, "嚚梧蔡嚚?]),  # noqa: RUF001
    ],
)
def test_normalize(form, expected, any_string_dtype):
    ser = Series(
        ["ABC", "嚗∴慰嚗?, "嚗?嚗?, np.nan, "嚚梧蔡嚚?],  # noqa: RUF001
        index=["a", "b", "c", "d", "e"],
        dtype=any_string_dtype,
    )
...
def test_normalize_index():
    idx = Index(["嚗∴慰嚗?, "嚗?嚗?, "嚚梧蔡嚚?])  # noqa: RUF001
    expected = Index(["ABC", "123", "?Ｕ??])
