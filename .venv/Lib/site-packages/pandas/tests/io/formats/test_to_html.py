...
def test_to_html_regression_GH6098():
    df = DataFrame(
        {
            "cl矇1": ["a", "a", "b", "b", "a"],
            "cl矇2": ["1er", "2癡me", "1er", "2癡me", "1er"],
            "donn矇es1": np.random.default_rng(2).standard_normal(5),
            "donn矇es2": np.random.default_rng(2).standard_normal(5),
        }
    )

    # it works
    df.pivot_table(index=["cl矇1"], columns=["cl矇2"])._repr_html_()
