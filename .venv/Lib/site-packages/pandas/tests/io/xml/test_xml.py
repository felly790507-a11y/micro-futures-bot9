...
def test_string_charset(parser):
    txt = "<銝剜?璅惜><row><c1>1</c1><c2>2</c2></row></銝剜?璅惜>"
...
def test_file_charset(xml_doc_ch_utf, parser):
    ...
    df_expected = DataFrame(
        {
            "??: [
                "?? ?亦??舫???湧 雿甇??(Sorry, this is Big5 only)?單迤",
                "???Ｙ???喟敺?鈭行?雿?批?喳??誑銝?,
                "???Ｙ?抒??鈭行?雿????交??∪瘣?鈭行??批????,
            ],
            "蝑?: [
...
def test_style_charset():
    pytest.importorskip("lxml")
    xml = "<銝剜?璅惜><row><c1>1</c1><c2>2</c2></row></銝剜?璅惜>"
