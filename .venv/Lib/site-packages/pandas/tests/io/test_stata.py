...
class TestStata:
    ...
    def test_read_dta18(self, datapath):
        ...
        expected = DataFrame.from_records(
            [
                ["Cat", "Bogota", "Bogot獺", 1, 1.0, "option b ?nicode", 1.0],
                ["Dog", "Boston", "Uzunk繹pr羹", np.nan, np.nan, np.nan, np.nan],
                ["Plane", "Rome", "Troms繪", 0, 0.0, "option a", 0.0],
                ["Potato", "Tokyo", "El璽z覺?", -4, 4.0, 4, 4],  # noqa: RUF001
...
"""
    ...
    def test_invalid_file_not_written(self, version):
        content = "Here is one __嚙稻_ Another one __繚__ Another one __翻__"
    ...
    def test_strl_latin1(self):
        ...
        output = DataFrame(
            [["pandas"] * 2, ["臘璽???禮"] * 2], columns=["var_str", "var_strl"]
        )
...
the string values returned are correct."""
        ...
        expected = DataFrame([["D羹sseldorf"]] * 151, columns=["kreis1849"])
    ...
    def test_utf8_writer(self, version):
        cat = pd.Categorical(["a", "帣", "?"], ordered=True)
        data = DataFrame(
            [
                [1.0, 1, "幓?, "幓 relatively long ?tring"],
                [2.0, 2, "幓?, ""],
                [3.0, 3, "幓?, None],
            ],
            columns=["?", "帣", "?", "strls"],
        ...
        data_label = "幓廘a-label"
        value_labels = {"帣": {1: "label", 2: "疆繪疇", 3: "?ot valid latin-1"}}
        data["帣"] = data["帣"].astype(np.int32)
        with tm.ensure_clean() as path:
            ...
            data["帣"] = (
                data["帣"].replace(value_labels["帣"]).astype("category").cat.as_ordered()
            )
