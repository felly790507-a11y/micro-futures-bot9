...
class StylerRenderer:
    ...
    def format(
        ...
        >>> df.style.format({0: '{:.2f}', 1: '瞿 {:.1f}'}, na_rep='MISS', precision=1)
        ...  # doctest: +SKIP
                 0      1     2
        0    MISS   瞿 1.0     A
        1    2.00    MISS   3.0
        ...
        >>> pseudo_css = "number-format: 0禮[Red](0)禮-禮@;"
    ...
    def wrapper(x):
        if is_float(x) or is_integer(x) or is_complex(x):
            if decimal != "." and thousands is not None and thousands != ",":
                return (
                    formatter(x)
                    .replace(",", "禮_禮-")  # rare string to avoid "," <-> "." clash.
                    .replace(".", decimal)
                    .replace("禮_禮-", thousands)
                )
...
def _parse_latex_table_styles(table_styles: CSSStyles, selector: str) -> str | None:
    ...
    Notes
    -----
    The replacement of "禮" with ":" is to avoid the CSS problem where ":" has structural
    ...
    for style in table_styles[::-1]:  # in reverse for most recently applied style
        if style["selector"] == selector:
            return str(style["props"][0][1]).replace("禮", ":")
    return None
...
def _escape_latex(s: str) -> str:
    ...
    """
    return (
        s.replace("\\", "ab2禮=禮8yz")  # rare string for final conversion: avoid \\ clash
        .replace("ab2禮=禮8yz ", "ab2禮=禮8yz\\space ")  # since \backslash gobbles spaces
        .replace("&", "\\&")
        .replace("%", "\\%")
        .replace("$", "\\$")
        .replace("#", "\\#")
        ...
        .replace("ab2禮=禮8yz", "\\textbackslash ")
    )
...
def _math_mode_with_dollar(s: str) -> str:
    ...
    return "".join(res).replace(r"rt8禮=禮7wz", r"\$")
...
def _math_mode_with_parentheses(s: str) -> str:
    ...
    str :
        Escaped string
    """
    s = s.replace(r"\(", r"LEFT禮=禮6yzLEFT").replace(r"\)", r"RIGHTab5禮=禮RIGHT")
    res = []
    for item in re.split(r"LEFT禮=禮6yz|ab5禮=禮RIGHT", s):
...
def _escape_latex_math(s: str) -> str:
    ...
    if len(mode) == 0:
        return _escape_latex(s.replace(r"rt8禮=禮7wz", r"\$"))
    if s[mode[0]] == r"$":
        return _math_mode_with_dollar(s.replace(r"rt8禮=禮7wz", r"\$"))
    if s[mode[0] - 1 : mode[0] + 1] == r"\(":
        return _math_mode_with_parentheses(s.replace(r"rt8禮=禮7wz", r"\$"))
    else:
        return _escape_latex(s.replace(r"rt8禮=禮7wz", r"\$"))
